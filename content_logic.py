...
# Load decoders from pickle files
...
def content_recommender(user_input, df, feature_df, top_n=5, input_type='title'):
    ...
    if input_type == 'title':
        ...
        selected_genres = [col for col in feature_df.columns 
                          if col not in ['runtimeMinutes', 'director', 'originalLanguage'] 
                          and selected_movie.get(col, 0) == 1]
        ...
        for idx in range(len(similarities)):
            ...
            movie_genres = [col for col in feature_df.columns 
                           if col not in ['runtimeMinutes', 'director', 'originalLanguage'] 
                           and movie.get(col, 0) == 1]
...
def get_director_decoder():
    return director_decoder
